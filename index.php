<!DOCTYPE html>
<html lang="Es">
	<head>
		<meta charset="utf-8"/>
		<title>Asociación de Auxilio Póstumo del Empleado de Salud Pública - ASAPESPU</title>
		<link rel="stylesheet" href="css/contindex.css">
		<link type="text/css" rel="stylesheet" href="anuncios/anuncio.css" />
        <!--Slider-->
        <link rel="stylesheet" href="dist/sheetslider.min.css"/>
        <script src="anuncios/jquery-1.2.3.min.js"></script>
		<script src="anuncios/anuncio.js"></script>
        <!--SCRIPT-->
		<?php include 'inc/header-common.php'; ?>
	</head>
	<body id="inicio">
		<div class="rescontainer">
			<div class="hidcontainer">
	
				<!-- -ENCABEZADO Y MENU-->
				<?php include 'inc/header.php'; ?>				
				<!-- INTRODUCCION 
				<div class="box introd">
					<div class="caja">
						<div class="nuevo_">
						  <p style="text-align: center">¡IMPORTANTE!</p>
						</div>
						 <h2> - ASUETOS POR FIN DE AÑO - </h2>
						 <P>
						 	A todos nuestros asociados se les informa que los dias <strong>Martes 24</strong>, <strong>Miercoles 25</strong>, <strong>Martes 31</strong> de <strong>Diciembre del 2019</strong> y <strong>Miercoles 01</strong> de <strong>Enero del 2020</strong> las instalaciones de ASAPESPU permaneceran cerradas por las festifidades de fin de Año, por lo que les agradecemos su comprensión. Representantes Departamentales, Junta Directiva, Comisiones, Gerencia y Planta Administrativa les desea una<strong> FELIZ NAVIDAD Y PROSPERO AÑO NUEVO</strong> deseando que Dios bendiga a todo este gremio Salubrista.
						 </P><br>
						 <p style="text-align: center; size: 2e;"><strong>Para verificar estados de cuenta en linea deben  solicitar usuario y contraseña para ingresar a la plataforma, esta solicitud únicamente la puede realizar el asociado.</strong></p><br>
						 <p>Para ingresar deben de dirigirse al menú principal, en el botón de <strong>Aplicaciones</strong> y dar clic en el botón de <strong>Estado de Cuenta en Línea</strong>.</p>
						 
					</div>
						
						<strong>Sistema De Estado De Cuenta En Línea</strong>

						<p>Somos una entidad guatemalteca de carácter civil, privada, no lucrativa, mutualista, ajena a toda cuestión de orden
						político partidista, sindical o religiosa, con personalidad jurídica reconocida legalmente, constituida por plazo 
						indefinido, conformada por todas las personas que por nombramiento, contrato o planilla prestan sus servicios al 
						Ministerio de Salud Pública y Asistencia Social y sus dependencias en todo el territorio nacional, así como los ex 
						empleados que han cesado en sus funciones por renuncia, destitución o jubilación toda vez cumplan con la aportación 
						de sus cuotas ordinarias mensuales.</p>
				</div> -->
                <!--Slider -->
                <?php include "inc/slider.php"; ?>
                <!-- -TITULO-	-->
				<div class="box tit_contenido">
					<div class="tit_sec_not">
						<h1 class="deco">Noticias</h1>
					</div>
				</div>
				<!-- -CONTENIDO-	-->
				<div class="box cua_contenido"> 
					<div class="cont_centrado">

						<div class="box noticia_w">
							<div class="fond_titulo">
								<h3>¡ASOCIADOS EN GENERAL!</h3>
							</div>
							<p>Para los asociados que se encuentren en el siguiente listado es necesario que se comuniquen al departamento de informática para solventar asuntos de su interés al Pbx. 2200-1515 Ext. 106/135</p><br>
							<p >Si desea verificar si se encuentra dentro del listado, deberá primero presionar el botón <strong>"Ver Listado"</strong>, dentro del documento deberá presionar las teclas <strong>Ctrl</strong> más <strong> f </strong> y deberá escribir sus nombres o apellidos para realizar la búsqueda.</p><br><br><br>
							<a class="listado" href="download/departamento de informatica/reporte_morosos_diciembre20.pdf" target="_Blank">Ver Listado</a>
						</div>
						<div class="box noticia_w">
							<div class="fond_titulo">
								<h3>¡ASOCIADOS JUBILADOS!</h3>
							</div>
							<p>A todos los asociados que han solicitado el descuento automático por concepto de Auxilio Póstumo (descuento de la cuota fija ordinaria mensual de la pensión por jubilación), se les informa que en el listado adjunto se encuentran los asociados a los cuales no se les realizo el descuento automático del mes de noviembre del año en curso, para solventar su situación debe comunicarse al PBX. 2200-1515 Ext. 106/135</p><br>
							<p>Si desea verificar si se encuentra dentro del listado, deberá primero presionar el botón <strong>"Ver Listado"</strong>, dentro del documento deberá presionar las teclas <strong>Ctrl</strong> más <strong> f </strong> y deberá escribir sus nombres o apellidos para realizar la búsqueda.</p><br>  
							<a class="listado" href="download/departamento de informatica/jubilados_diciembre20.pdf" target="_Blank">Ver Listado</a>
						</div>

						<!--<div class="box noticia">
							<div class="fond_titulo">
								<h3>SALON DE EVENTOS</h3>
							</div>
							<p>Celebre con nosotros, ese evento tan especial... ¡Aproveche nuestras nuevas tarifas! Asociados: Q 3,000.00 No asociados: 3,500.00 Para mayor información comuníquese a nuestra recepción 2200-1515 Ext. 101</p>
						</div>-->
						<div class="box noticia">
							<div class="fond_titulo">
								<h3>NUESTROS REPRESENTANTES DEPARTAMENTALES</h3>
							</div>
							<p>La asociacion cuenta con representates en toda la republica para darle a nuestros asociados la posibilidad y comodidad de hacer sus tramites en la asociación, recomendamos a nuestros asociados que a traves de ellos puedan hacer los siguientes tramites:
								Depto. Secretaria General
								Inscripciones
								Cambios de beneficiarios
								Aviso de fallecimientos
								Actualizaciones de expedientes</p>
						</div>
					</div>
				</div>
				<!-- 	-FOOTER-	-->
				<?php include 'inc/footer.php'; ?>
				<?php include 'inc/footer-index.php'; ?>
                <script src="dist/sheetslider.min.js"></script>
            </div>
		</div>
	</body>
</html>